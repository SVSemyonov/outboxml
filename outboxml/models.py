...
class CatboostOverGLMModel(BaseWrapperModel, RegressorMixin, BaseEstimator):
    ...
    def predict(self, X):
        self._glm_prediction = self.__predict_glm(X)

        prediction = self._glm_prediction * self._model_ctb.predict(
...
class ModelsWrapper(BaseWrapperModel):
    ...
    def __init__(self, data_subsets: dict, models_configs: List[ModelConfig]):
...
class StatsModelsEstimator(RegressorMixin, BaseEstimator):
    ...
    def predict(self, X):
        ...
        prediction = self.sm_model.predict(dataset_sm[chain(features_numerical, features_categorical)])
        return prediction
