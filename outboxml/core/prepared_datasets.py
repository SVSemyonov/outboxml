    ...
    def prepare_dataset(self, data: pd.DataFrame,  index_train: pd.Index, index_test:pd.Index,
        ...
        if new_features is not None:
            for feature_num in new_features[FeatureTypesForSelection.numeric]:
                model_config.features.append(self._numerical_feature(feature_num, data[feature_num], features_params[feature_num])
                                    )
            for feature_cat in new_features[FeatureTypesForSelection.categorical]:
                model_config.features.append(self._categorical_feature(feature_cat, data[feature_cat],features_params[feature_cat]))
