...
class ResultExport:
       ...
       """
        ...
        for result in self.result.values():
            ...
            for key in res_dict.predictions:
                ...
                if key == 'train':
                    y_true = res_dict.datasubset.y_train
                    x = res_dict.datasubset.X_train
                    ...
                    y_true = res_dict.datasubset.y_test
                    x = res_dict.datasubset.X_test
            ...
            save_results(config=self.config,
                         ...
                         features_numerical=res_dict.datasubset.features_numerical,
                         features_categorical=res_dict.datasubset.features_categorical,
    ...
    def grafana_export(self, project_name: str = None, date_time=datetime.utcnow()):
    ...
    def metrics_df(self, model_name: str, metrics_dict: dict = None, train_test: str = 'train',
        ...
        for key in metrics_dict.keys():
            for key_slice in metrics_dict[key].keys():
                ...
                metrics1[key] = {}
                metrics1[key]['Value'] = metrics_dict[key][new_key]

            df1 = pd.DataFrame(metrics1).transpose()['Value'].apply(pd.Series)
        df = df1.fillna(0)
        df = df.transpose()
        return df
    ...
    def plots(self, model_name: str,
        ...
        if user_plot_func is not None:
            user_plot_func(y_graph, model_name, features_categorical, features_numerical, bins)
        else:
    ...
    def __save_predictions(self, y_true, prediction: pd.DataFrame, model_name: str, key: str, path=None,
        ...
        # df.columns = [x.name, 'y_true', 'prediction']
        #  df.to_parquet(str(path) + '/' f"{model_name}_predictions_{key}.gzip")
        return df
...
class GrafanaExport:
    ...
    def load_data_to_db(self):
        ...
        self.df.to_sql(self.table_name, schema=self.schema, con=self.__connection, if_exists='append')
