...
class FeatureSelection(TestCase):
    ...
    def test_BaseFS(self):
        data = BaseFS(data=self.data, new_features_list=self.feature_for_research,
                      parameters=self._fs_config,
                      train_ind=self.dsManager.index_train,
                      test_ind=self.dsManager.index_test,
                      target=self.dsManager.Y['SURVIVED'],
                      data_prepare_interface=FeatureSelectionPrepareDataset(model_config=self.dsManager._models_configs[
                          0]),
                      feature_selection_interface=FeatureSelectionInterface(feature_selection_config=self._fs_config,
                                                                            objective='binomial')
                      ).select_features(params={"iterations": 30})
...
class HPTune(TestCase):
    ...
    def test_hp_tune_catboost(self):
        ...
        params = HPTuning(ds_manager=self.ds_manager, folds_num_for_cv=5, ).best_params(model_name='first',
                                                                                        trials=5,
                                                                                        direction='maximize',
    ...
    def test_hp_tune_glm(self):
        ...
        params = HPTuning(ds_manager=self.ds_manager, folds_num_for_cv=5, ).best_params(model_name='second',
                                                                                        trials=5,
                                                                                        direction='maximize',
                                                                                        parameters_for_optuna_func=parameters_for_optuna)
        self.assertIsInstance(params, dict)
...
class AutoMLTest(TestCase):
    ...
    def setUp(self):
        ...
        self.ds_manager2 = deepcopy(self.ds_manager1)
