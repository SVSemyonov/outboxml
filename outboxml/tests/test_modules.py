...
class TestTitanicDS(TestCase):
    ...
    def test_getTrainResults(self):
        ...
        self.assertEqual(len(results1), self.dsManager.data_config.data.targetslices[0]['slices'] + 1)
    ...
    def test_api_models_and_metrics(self):
        ...
        results = self.dsManager.fit_models(resultDics, classification=False)
        self.assertIsInstance(results, dict)


    def test_baseline_classification(self):
        self.assertEqual(len(self.dsManager_base.fit_models(classification=True).keys()),6)
    ...
    def test_predict(self):
        ...
        self.assertEqual(model_res.datasubset.X_test.shape, ( 891, 4))
        self.assertEqual(len(model_res.datasubset.features_categorical), 0)
        self.assertEqual(len(model_res.datasubset.features_numerical), 4)


    def test_default_models(self):
        self.dsManager.get_TrainDfs(model_name='first')
        datasubset = self.dsManager.data_subsets['first']
