...
class DataframeForPlots:
    """Класс подготовки датафреймов для графиков"""
    ...
    def df_for_plots(self, result: DSManagerResult, features: list = None, use_exposure: bool = False, only_test: bool = False) -> \
        ...
        features_categorical = data.datasubset.features_categorical
        if features_categorical == []: features_categorical = ['No features']
        features_numerical = data.datasubset.features_numerical
        if features_numerical == []: features_numerical = ['No features']
        exposure_test = data.datasubset.exposure_test
        exposure_train = data.datasubset.exposure_train
        ...
        if not only_test:
            X = pd.concat([data.datasubset.X_test, data.datasubset.X_train])
            y_true = pd.concat([data.datasubset.y_test, data.datasubset.y_train])
            y_pred = pd.concat([data.predictions['test'], data.predictions['train']])
        else:
            X = data.datasubset.X_test
            y_true = data.datasubset.y_test
...
class MLPlot:
    """Основной класс построения графиков модели и факта
    основной метод make вызывает вспомогательные методы построения графиков

    Methods: cohort_plot, factors_plot, loss_plot, feature_plot
    ...
    def feature_plot(self, use_exposure: bool = True):
        ...
        for feature in features:
            ...
            figure = fig.prepare_plot(x_name=feature)
            if self.show:
                figure.show()
...
class FactorsPlot:
    """Класс построения графиков по фактору. Основной метод male_plots"""
    def __init__(self,
                 show: bool = True
                 ):
        self.show = show
...
class CompareModelsMetrics:
    ...
    def compare_metrics(self, model_name: str, business_metric=None, only_main: bool = False):
        ...
        for key in res_dict_main.metrics['train'].keys():
            ...
            for key_slice in res_dict_main.metrics['train'][key].keys():

                val = key_slice.split(sep='_model')
                if val[0] == model_name:
                    new_key = key_slice
                ...
                metrics1[key]['first model train'] = res_dict_main.metrics['train'][key][new_key]
                metrics1[key]['first model test'] = res_dict_main.metrics['test'][key][new_key]
                try:
                    metrics2[key]['second model train'] = res_dict_to_compare.metrics['train'][key][new_key]
                    metrics2[key]['second model test'] = res_dict_to_compare.metrics['test'][key][new_key]
                except KeyError:
...
class PlotlyWrapper:
    ...
    def __init__(self, model_name: str,
        ...
        self.names = {'fact': 'Факт',
                      'model1': 'Новая модель',
                      'model': 'Новая модель',
                      'model2': 'Старая модель',
                      'exposure': 'Экспозиция'
    ...
    def prepare_plot(self, x_name: str = "Группы по таргету"):
        ...
        self.figure.update_yaxes(title_text="Экспозиция", side="right", secondary_y=True)
        return self.figure
